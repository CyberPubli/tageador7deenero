...
window.chatObserver = {
  ...
  iterateObserveChats(chatDivs) {
    ...
    async function procesarChatActual() {
      ...
      let urlInfo = await window.urlDetector.extractUrlFromChat();
      ...
      if (urlFinal !== 'Sin URL' && !urlInfo.letraCampana) {
        ...
        self.callbackReanudar = async () => {
          ...
          const urlInfoActualizada = await window.urlDetector.extractUrlFromChat();
  ...
  async tagearMultiplesEnObservacionesAsync(nomenclaturas, chatNum) {
    ...
    for (const nomItem of nomenclaturas) {
      ...
      // ¿Ya existe?
      let existe = codigos.find(c => c.replace(/!$/, '') === nomenclaturaSinSigno);
      ...
      if (!existe) {
        ...
        console.log(`      ✓ "${nomenclatura}" ya existe`);
      }
    }
